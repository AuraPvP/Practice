...
public class LobbyParkourListener implements Listener {
    ...
    @Getter public static Map<UUID, Parkour> parkourMap = Maps.newHashMap();
            ...
            if (block.getType() == Material.IRON_PLATE) {
                ...
                if (block.getRelative(BlockFace.DOWN).getType() == Material.EMERALD_BLOCK) {
                    ...
                    if (parkour == null) {
                        ...
                        player.sendMessage(ChatColor.GREEN + "You've started the parkour challenge!");
                    }

                } else if (block.getRelative(BlockFace.DOWN).getType() == Material.REDSTONE_BLOCK) {
                    Parkour parkour = parkourMap.get(player.getUniqueId());
                    if (parkour != null) {
                        player.sendMessage(ChatColor.GREEN + "You've finished the parkour challenge!");
                        parkour.getCheckpoint().ring(player);
                        parkourMap.remove(player.getUniqueId());
                    }
                } else if (block.getRelative(BlockFace.DOWN).getType() == Material.BEDROCK) {
                    Parkour parkour = parkourMap.get(player.getUniqueId());
                    if (parkour != null) {
                        player.sendMessage(ChatColor.RED + "You've cancelled the parkour challenge!");
                        parkour.getCheckpoint().ring(player);
                        parkourMap.remove(player.getUniqueId());
                        }
                }
                ...
                if (parkour != null) {
                    ...
                    if (!checkpoint.getLocation().equals(block.getLocation())) {
                        ...
                        parkour.getCheckpoint().ring(player);
                        player.sendMessage(ChatColor.YELLOW + "You've reached a new" + PotPvPND.getInstance().getDominantColor() + " checkpoint" + ChatColor.YELLOW + "!");
                    }
                }
            }
        }
