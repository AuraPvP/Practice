...
public final class PostMatchInvLang {
    ...
    static final String LINE = ChatColor.DARK_GRAY.toString() + ChatColor.STRIKETHROUGH + "";
    static final String INVENTORY_HEADER = CC.translate("&d&lMatch Status &7&o(click name)");
    ...
    private static TextComponent clickToViewLine(UUID member) {
        String memberName = PotPvPND.getInstance().getUuidCache().name(member);
        TextComponent component = new TextComponent();

        component.setText(memberName);
        component.setColor(ChatColor.WHITE);
        component.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(ChatColor.WHITE + "Click to view inventory of " + PotPvPND.getInstance().getDominantColor() + memberName).create()));
