                ...
                if (type.parameterCount > 1) {
                    for (i in 1 until type.parameterCount) {
                        ...
                        if (parameter.isAnnotationPresent(Param::class.java)) {
                            ...
                            val data = ParameterData(param.name, param.defaultValue, parameter.type, param.wildcard, i, hash, PotPvPND.getInstance().commandHandler.parameterTypeMap[parameter.type]?.javaClass)
                            allParams.add(data)
                        } else {
